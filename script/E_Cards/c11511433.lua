...
function s.initial_effect(c)
	...
	Synchro.AddProcedure(c,aux.FilterBoolFunction(Card.IsSetCard,0xffb),1,1,Synchro.NonTunerEx(Card.IsCode,11511413),1,1)
	--special summon
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if mg:GetCount()>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		...
		if Duel.SpecialSummon(mg,0,tp,tp,false,false,POS_FACEUP) then 
			...
			mg=mg:Filter(aux.NecroValleyFilter(s.mgfilter),nil,e,tp,e:GetHandler(),mg,LOCATION_HAND)
