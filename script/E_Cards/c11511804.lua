--Sarinx, the Xerxian Beast
local s,id=GetID()
function s.initial_effect(c)
	--ritual level
	Ritual.AddWholeLevelTribute(c,aux.FilterBoolFunction(Card.IsCode,11511808))
...
s.listed_names={11511808}
function s.valcon(e,re,r,rp)
	return (r&REASON_BATTLE)~=0
end
function s.filter(c)
	return c:IsSetCard(0xff7) and c:IsType(TYPE_RITUAL) and c:IsAbleToHand()
end
function s.filterM(c)
	return c:IsMonster() and s.filter(c)
end
