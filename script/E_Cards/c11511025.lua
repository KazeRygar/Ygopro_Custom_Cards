...
function s.initial_effect(c)
	--xyz summon
	Xyz.AddProcedure(c,aux.FilterBoolFunction(Card.IsSetCard,0xfff),1,2)
	c:EnableReviveLimit()
	--cannot be target
...
function s.filter1(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0xfff)  and c:GetLevel()>0
		and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,c:GetLevel())
end
